...
/**
 * Standalone utility to capture baseline simulation results for all circuits.
 * This should be run BEFORE any refactoring begins to establish the golden reference.
 *
 * Usage:
 *   java -cp target/test-classes:target/classes ch.technokrat.regression.BaselineCapture
 *
 * Or via Maven:
...
public class BaselineCapture {
    ...
    private static final Logger LOGGER = Logger.getLogger(BaselineCapture.class.getName());
    ...
    public static void main(String[] args) {
        ...
        BaselineCapture capture = new BaselineCapture();
    ...
    private void parseOptions() {
        ...
        LOGGER.info("Baseline Capture Configuration:");
        ...
        for (int i = 0; i < circuits.size(); i++) {
            ...
            try {
                ...
                if (result.getSignals().isEmpty()) {
                    LOGGER.warning("  No signals captured - circuit may not have scopes");
                    failureCount++;
                    continue;
                }
    ...
    private void verifyBaseline(File circuitFile, String circuitName, SimulationResult firstRun)
        ...
        SimulationResult secondRun = GoldenReferenceTestHarness.runCircuitSimulation(circuitFile);
