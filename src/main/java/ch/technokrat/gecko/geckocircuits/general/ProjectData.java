...
public class ProjectData implements Serializable {
    ...
    public transient List<String> _optimizerNames = new ArrayList<String>();
    public transient List<Double> _optimizerData = new ArrayList<Double>();
...
//        geckoOpt.exportASCII(asc);
        ...
        final int globalDataSize = NetlistControl.globalData.getRowLength();
        String[] signalNames = new String[globalDataSize];
        for (int i = 0; i < globalDataSize; i++) {
            signalNames[i] = NetlistControl.globalData.getSignalName(i);
        }
    ...
    private int importASCII(String[] ascii, final boolean isBackupRead) {
        ...
        importBasicSettings(tokenMap);
        importWorksheetSize(tokenMap);
        importDisplayAndScriptSettings(tokenMap, isBackupRead);
        loadCircuitComponents(tokenMap);
        loadGeckoFileManager(tokenMap);

        return uniqueFileId;
    }

    private void importBasicSettings(TokenMap tokenMap) {
        if (tokenMap.containsToken("tDURATION")) {
            ...
            GlobalFilePathes.datnamAbsLoadIPES = nameString.substring((new String("path ").length()));
            fontSize = tokenMap.readDataLine("fontSize", fontSize);
            final String fontString = tokenMap.getLineString("fontTyp", "fontTyp ");
            _fontTyp = fontString.substring((new String("fontTyp ")).length());
    ...
    private void importWorksheetSize(TokenMap tokenMap) {
    ...
    private void importDisplayAndScriptSettings(TokenMap tokenMap, boolean isBackupRead) {
        ...
        if (!isBackupRead && tokenMap.containsToken("dataContainerSignals[]")) {
            ...
            for (int row = 0; row < sigNames.length; row++) {
                // TODO ??? NetlistControl.globalData.setSignalName(row, sigNames[row]);
            }
        }
    }

    private void loadCircuitComponents(TokenMap tokenMap) {
        ...
        for (TokenMap elementLKMap = tokenMap.getSpecialBlockTokenMap("e");
            ...
            AbstractBlockInterface newBlock = AbstractTypeInfo.fabricFromFile(CircuitType.getFromIntNumber(typ), elementLKMap);
        ...
        for (TokenMap thermTokenMap = tokenMap.getSpecialBlockTokenMap("eTH"); thermTokenMap != null;
            ...
            _allSheetComponents.add(AbstractTypeInfo.fabricFromFile(CircuitType.getFromIntNumber(typ), thermTokenMap));
        }
        ...
        for (TokenMap controlTokenMap = tokenMap.getSpecialBlockTokenMap("c");
            ...
            try {
                if (typ == SpecialType.TEXTFIELD.getTypeNumber()) { // this is for backwards-compatiblity before v 1.62
                    ...
                    _allSheetComponents.add(AbstractTypeInfo.fabricFromFile(ControlComponentType.getFromIntNumber(typ), controlTokenMap));
                }
            } catch (Exception ex) {
    ...
    private void loadGeckoFileManager(TokenMap tokenMap) {
    ...
    // // datnamAbsLoadDETAIL ... saved path+name of a file that contains additional information, e.g. Semiconductor info or replacement models like TH_MODUL
    // // return string ... new current absolute path+name of the file with the additional information
    //
    @Deprecated
    public static String localizeRelativePath(String datnamAbsolutIPES, String datnamAbsLoadDETAIL) {
        ...
        //-------------------------
        // // (2) The old file 'datnamAbsLoadDETAIL' was not found.
        // // Has the path structure changed and does the new path need to be found?
        // // (a) Search the subdirectories of *.ipes -->
        try {
