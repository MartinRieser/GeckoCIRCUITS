...
import ch.technokrat.gecko.geckocircuits.nativec.ControlNativeC;
...
public enum ControlComponentType implements AbstractComponentType {
    ...
    C_ABS(32, ControlAbsolutValue.tinfo),
    ...
    ControlComponentType(final int initValue, final AbstractTypeInfo typeInfo) {
        _intValue = initValue;
        _typeInfo = typeInfo;
        assert _typeInfo != null;
        _typeInfo.addParentEnum(this);
    }
    private static Map<Integer, ControlComponentType> _backwardMap;

    public static ControlComponentType getFromIntNumber(final int intNumber) {

        if (_backwardMap == null) {
            _backwardMap = new HashMap<Integer, ControlComponentType>();
            for (ControlComponentType typ : values()) {
