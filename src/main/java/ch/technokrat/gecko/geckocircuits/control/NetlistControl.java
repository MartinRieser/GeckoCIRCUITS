...
import ch.technokrat.gecko.geckocircuits.circuit.NetlistGeneral;
...
public final class NetlistControl {
    ...
    //-------------------------------------------
    private AbstractControlCalculatable[] controlCalc;
    private final List<ControlBlock> elementsControl = new ArrayList<ControlBlock>();
    private PotentialArea[] potLab;
    private String[] labelListeControlKnoten;  // // index is node number and string entry is associated label
    public ControlBlock[] _orderedControlBlocks;
    ...
    private ControlBlock[] _sortedControlWONonCalculatables;
    ...
    public ControlBlock[] getElementCONTROL() {
        return elementsControl.toArray(new ControlBlock[0]);
    }

    public ControlBlock[] getEnabledElementCONTROL() {
        List<ControlBlock> enabledControls = new ArrayList<ControlBlock>();
        for (ControlBlock ctrl : elementsControl) {
            if (ctrl.isCircuitEnabled() == Enabled.ENABLED) {
                enabledControls.add(ctrl);
            }
        }
        return enabledControls.toArray(new ControlBlock[0]);
    }

    public String[] getLabelListeControlKnoten() {
        return labelListeControlKnoten;
    }
    ...
    private void connectPotentialLabels(final NetlistGeneral nlA) {
        ...
        for (AbstractBlockInterface elem : nlA.getElemente()) {
            if (elem != null && elem.isCircuitEnabled() != Enabled.DISABLED && !(elem instanceof ControlTERMINAL)) {
                elementsControl.add((ControlBlock) elem);
            }
        }
        ...
        for (int i1 = 0; i1 < elementsControl.size(); i1++) {
            ...
            // Endknoten:
            List<AbstractTerminal> endNodes = elementsControl.get(i1).YOUT;  // // x values ​​(ScematicEntry) of the end nodes
        ...
        labelListeControlKnoten = new String[potLab.length];
        for (int i1 = 0; i1 < potLab.length; i1++) {
            labelListeControlKnoten[i1] = potLab[i1].getLabel();
        }
    }
    ...
    public void initializeConnections() {
        ...
        for (int ii = 0; ii < elementsControl.size(); ii++) {
            ControlBlock reg = elementsControl.get(ii);
            for (int i = 0; i < reg.YOUT.size(); i++) {
                boolean connectionDone = false;
                for (ControlBlock reg2 : elementsControl) {
        ...
        for (ControlBlock reg : elementsControl) {
    ...
    private void removeMuxAndDemux() {
        ...
        for (ControlBlock block : elementsControl.toArray(new ControlBlock[0])) {
            ...
            if (demuxes.containsKey(muxEntry.getKey())) {
                ...
                for (ControlDemux demux : demuxes.get(muxEntry.getKey())) {
                    ...
                    for (int i = 0; i < mux.XIN.size(); i++) {
                        ...
                        for (ControlBlock regelblock : elementsControl) {
...
////                    Logger.getLogger(NetlistControl.class.getName()).log(Level.SEVERE, null, ex);
////                }
////        }
////        try {
...
//        }
        for (int i = 0; i < _sortedControlBlocksNew.length; i++) {
            _sortedControlBlocksNew[i].calculateYOUT(deltaT);
        }        
...
//                    if(value != value) {
//                        for(int k = 0; k < _sortedControlWONonCalculatables.length; k++) {
//                            if(_sortedControlWONonCalculatables[k]._calculator == calc) {
//                                System.out.println("calculator " +  _sortedControlWONonCalculatables[k].getStringID() + " " + i + " " + j);
//                            }
//                        }                        
//                    }
//                    
...
//        }
    ...
    // // Is only called in 'SimulationKernel', where the simulation is actually started
    // // ... this avoids that when setting up the control in the SchematicEntry every time an element or a parameter
...
//        }        
    ...
    public void createControlCalculators() {
        ...
        List<ControlBlock> sortedWONonCalculators = new ArrayList<ControlBlock>();
        ...
        _sortedControlWONonCalculatables = sortedWONonCalculators.toArray(new ControlBlock[0]);
    ...
    private void setGlobalDatacontainer() {
        ...
        int maxNumOfInput = 0;
        for (int i = 0; i < _orderedControlBlocks.length; i++) {
            final ControlBlock control = _orderedControlBlocks[i];
