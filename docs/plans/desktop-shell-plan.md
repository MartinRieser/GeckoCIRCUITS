...
**WP0.4 Script-block spike** — run `resources/projects/interleaved_pfc_50v.ipes` (typ-61 MCU script) through the REST jar alone; if polyglot is missing, add `org.graalvm.polyglot:polyglot` + `:js` to the rest-api pom and `-Dpolyglot.js.nashorn-compat=true` to the sidecar JVM args. Deliverable: JUnit `ScriptBlockSimulationIT` (runs in CI) proving script circuits simulate without the GUI jar.
